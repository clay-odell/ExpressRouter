...
beforeEach(function () {
  pickles = { name: "pickles", price: "$2.45" };
  items.push(pickles);
});
...
describe("PATCH /items/:name", function () {
  test("It should update an item's name and/or price", async function () {
    const resp = await request(app)
      .patch("/items/pickles")
      .send({ name: "newPickles", price: "$2.45" });
    expect(resp.statusCode).toBe(200);
    expect(resp.body).toEqual({
      updated: { name: "newPickles", price: "$2.45" },
    });
  });
});

describe("DELETE /items/:name", function () {
  test("It should delete an item and return a message", async function () {
    const resp = await request(app).delete("/items/pickles");
    expect(resp.statusCode).toBe(200);
    expect(resp.body).toEqual({message: "Deleted"});
  });
  test("It should return a 404 error if the item doesn't exist", async function () {
    const resp = await request(app).delete("/items/notpickles");
    expect(resp.body).toEqual({
      error: { message: "Item not found", status: 404 },
    });
  });
